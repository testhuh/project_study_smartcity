# -*- coding: utf-8 -*-
"""
smartcity_core.py — SmartCity GUI 연동용 Playwright Core
======================================================
GUI(smartcity_gui.py)에서 JSON 1줄을 stdin으로 받아 예약을 실행합니다.
...
중요 수정점
- GUI가 호출하던 smartcity_core.py 누락 문제 해결
- 기존 smartcity_book.py에서 정상 동작하던 예약 로직을 JSON payload 기반으로 변경
- SmartCity 구형 showModalDialog/더블클릭 팝업 대신 reservationCreateView.do 직접 진입
...
URL = ("https://smartcity.sec.samsung.net/common/roomreservation/reservation/"
       "reservationList.do?_menuId=AT4bCWcJAtEJCU_c&_menuF=true")
MY_URL = ("https://smartcity.sec.samsung.net/common/roomreservation/reservation/"
          "reservationMyList.do?_menuId=AT4bCWcJAtEJCU_c")
CREATE_URL = "https://smartcity.sec.samsung.net/common/roomreservation/reservation/reservationCreateView.do"

# GUI 위치값 → SmartCity locationId 매핑
...
def resolve_location_id_from_page(page, payload):
    """SmartCity 검색 select(#searchRoom)의 option 텍스트에서 R5/A타워/층을 찾아 locationId를 얻는다."""
...
def ensure_login(page):
    if "reservationList" in page.url:
        return True
    ...
    try:
        page.wait_for_url("**reservationList**", timeout=10000)
        emit("OK", "로그인 성공")
        return True
    except Exception:
...
def normalize_date(v: str) -> str:
    """GUI 입력 날짜를 SmartCity 검색창용 yyyy-mm-dd 로 정리."""
...
def set_search_date(page, date: str):
    """SmartCity 날짜 입력창/숨김필드/datepicker 값을 모두 요청 날짜로 강제 동기화."""
    date = normalize_date(date)
    page.wait_for_selector("#searchDate", timeout=20000)

    # SmartCity가 오늘 날짜로 다시 돌리는 경우가 있어, 화면에 있는 모든 날짜 관련 input을 같이 변경한다.
...
def _norm_room_name(v: str) -> str:
    """SmartCity 화면의 회의실 표기 흔들림 대응용 정규화."""
...
def _parse_add_args(ondbl):
    ...
    if not m:
        return None
    raw = m.group(1)
    # SmartCity _add 인자는 대부분 작은따옴표 문자열이므로 quote 기준 우선 파싱
...
def extract_create_params(page, room_match, start):
    ...
    # SmartCity 표가 가로 스크롤/부분 렌더링이라 14:00 컬럼을 DOM에서 못 잡아도
...
def _parse_my_reservations(page):
    """SmartCity 내 예약 표를 GUI 테이블 형식으로 변환한다."""
...
def fetch(payload):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            time.sleep(0.5)
            if "reservationMyList" not in page.url:
                if not ensure_login(page):
                    raise RuntimeError("로그인 실패")
                page.goto(MY_URL, wait_until="domcontentloaded", timeout=30000)
                time.sleep(0.5)
...
def open_status(payload):
    """GUI 팝업/파싱 없이 SmartCity '내 예약' 화면만 브라우저로 띄운다."""
